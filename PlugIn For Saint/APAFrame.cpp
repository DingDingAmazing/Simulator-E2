...
///
/*
step=0  按键前检查停车状态                控件触发          检查APAGears==P/D和APASpeed==0给出explain
step=1  按键时                           发送ODIEvent      检查APAGainDisplay==false给出explain
step=2  基于step1获取界面时初始化focus    DataCallback      检查APAGainDisplay==true给出explain             发送ODIIndication和ODIDynamicData
step=3  基于step2再次按键取消APA          发送ODIEvent      release display给出explain                      step置0
step=4  基于step2 touch选择泊车类型       DataCallback      检查给出APAParkMode[]给出explain
step=5  基于step2/4/7手动提供slot          控件触发           检查APAGears==D和APASpeed>0给出explain          发送ODIIndication和ODIDynamicData
step=6  基于step4/7搜索过程中R档退出      控件触发           检查APAGears==R给出explain                      发送ODIEvent和ODIIndication和ODIDynamicData
step=7  基于step5过车位继续搜索           OnTimer 15s触发    检查SlotFound==true切换回搜索界面 给出explain    SlotFound置0  发送ODIIndication和ODIDynamicData
...
step=13 直接判断异常情况退出APA           发送ODIEvent和ODIIndication和ODIDynamicData 给出explain step置0
